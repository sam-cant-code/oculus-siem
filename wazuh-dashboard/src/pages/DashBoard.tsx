...
export const Dashboard = () => {
  ...
  // --- 2. DYNAMIC DATA CALCULATION (No more mocks) ---
              ...
              <div key={`${alert.id}-${idx}`} className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-siem-border/10 hover:bg-white/5 transition-colors text-xs items-center font-mono">
                ...
                <div className="col-span-1">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                    alert.rule.level >= 12 ? 'border-red-500 text-red-400 bg-red-500/10' :
                    alert.rule.level >= 7 ? 'border-yellow-500 text-yellow-400 bg-yellow-500/10' :
                    'border-emerald-500 text-emerald-400 bg-emerald-500/10'
                  }`}>
                    {alert.rule.level}
                  </span>
                </div>
                <div className="col-span-2 text-siem-accent truncate">{alert.agent.name}</div>
                <div className="col-span-5 text-gray-300 truncate">{alert.rule.description}</div>
                <div className="col-span-2 text-gray-500 truncate">{alert.location}</div>
              </div>
            ))
          )}
        </div>
